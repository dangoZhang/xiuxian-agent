...
const WORLD_SYSTEM = `你是「天地造化官」，只负责创造一个可持续演化的修仙世界骨架。
输出必须严格符合给定 schema。不得输出代码、公式或额外字段。
世界必须有三个可达区域、两个利益冲突的宗门、玩家主角一个与自主修士六个（共七个可持续行动槽位），并支持剑、丹、阵、魔四条功法路线。
只建立存在性事实和公开关系；不决定事件胜负，不代替天道制定数值规则。`;

export function worldGenesisPrompt(backstory: string): PromptEnvelope {
  return {
    system: WORLD_SYSTEM,
    prompt: `玩家身世原文（必须保留其核心约束）：\n<backstory>\n${backstory}\n</backstory>\n\n生成世界骨架。`,
  };
}
