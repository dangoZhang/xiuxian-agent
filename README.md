...
玩家只需配置 Base URL、API Key 和模型。能力校验通过后，大模型自主生成世界、角色、规则和小说开篇。游戏页只保留小说式对话瀑布与一句话输入。
...
打开 `http://localhost:5173`，填写 Base URL、API Key 和模型名称，然后点击“开始”。服务端通过结构化输出、工具调用和流式文本校验后，模型会自动生成世界与小说开篇。API Key 只存在当前浏览器内存与服务端会话内存，不写入存档或浏览器存储。
